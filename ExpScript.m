...
P1Timings.trigger = GetSecs;
[~,~,~,P1Timings.instructions] = TextScreen(window,'Phase 1: Please wait for instructions',[1 1 1],'key');
[~,~,~,P1Timings.getready] = TextScreen(window,'Get Ready',[1 1 1],1.5);
...
for i = 1:numMVCTrials
    [~,volt,timing,P1Timings.MVC(i)] = TextScreen(window,'Squeeze!',[1 1 1],time,sensor,baseline);
    timingMVCTrial(i,:) = timing;
    voltMVCTrial(i,:) = volt;
    P1Timings.fixcross(i) = FixationCross(window,1+3*rand); %random duration btwn 1-4 sec
end
...
[~,~,~,P1Timings.end] = TextScreen(window,'End of Phase 1',[1 1 1],'key');
...
P2Timings.trigger = GetSecs;
[~,~,~,P2Timings.instructions] = TextScreen(window,'Phase 2: Please wait for instructions',[1 1 1],'key');
[~,~,~,P2Timings.getready] = TextScreen(window,'Get Ready',[1 1 1],1.5);
...
for i = AssocTrial'
    count = count+1;
    count_1 = count_1+1;
    [~,~,~,P2Timings.number(count)] = TextScreen(window,num2str(i),[1 1 1],2);
    [outcome,volt,timing,P2Timings.therm(count)] = ThermScreen(window,sensor,baseline,MVC,i/100,0.05,'vertical',time);
    voltAssocTrial(count,:) = volt;
    timingAssocTrial(count,:) = timing;
    outcomeAssocTrial(count) = outcome;
    if outcome == 1
        [~,~,~,P2Timings.outcome(count)] = TextScreen(window,'Success',[0 1 0],2);
    elseif outcome == 0
        [~,~,~,P2Timings.outcome(count)] = TextScreen(window,'Failure',[1 0 0],2);
    end
    P2Timings.fixcross(count) = FixationCross(window,1+3*rand);
    if count_1 == numAssocTrials
        count_2 = count_2+1;
        [~,~,~,P2Timings.rest(count_2)] = TextScreen(window,'Rest',[1 1 1],60);
        count_1 = 0;
    end
end
...
[~,~,~,P2Timings.end] = TextScreen(window,'End of Phase 2',[1 1 1],'key');

% save data
AssocFileName = fullfile(SubjectDir,'AssocPhase');
save(AssocFileName,'AssocTrial','voltAssocTrial','timingAssocTrial','P2Timings');
...
P3Timings.trigger = GetSecs;
[~,~,~,P3Timings.instructions] = TextScreen(window,'Phase 3: Please wait for instructions',[1 1 1],'key');
...
for i = 1:numTrialReps
    [~,~,~,P3Timings.getready(i)] = TextScreen(window,'Get Ready',[1 1 1],1.5);
    PercentMVClevels = [10 20 30 40 50 60 70 80];
    ...
    for j = PercentMVClevels_3_shuffle
        count = count+1;
        [outcome,volt,timing,P3Timings.therm(count)] = ThermScreen(window,sensor,baseline,MVC,j/100,0.05,'horizontal',time);
        voltRecallTrial(count,:) = volt;
        timingRecallTrial(count,:) = timing;
        outcomeRecallTrial(count) = outcome;
        [EffortReport,ReactTime,P3Timings.numline(count)] = NumberLineScreen(window);
...
[~,~,~,P3Timings.end] = TextScreen(window,'End of Phase 3',[1 1 1],'key');

% save data
RecallFileName = fullfile(SubjectDir,'RecallPhase');
save(RecallFileName,'RecallTrial','voltRecallTrial','timingRecallTrial','P3Timings');
...
if MRI == 0
    ...
    P4Timings.trigger = GetSecs;
    [~,~,~,P4Timings.instructions] = TextScreen(window,'Phase 4: Please wait for instructions',[1 1 1],'key');
    [~,~,~,P4Timings.getready] = TextScreen(window,'Get Ready',[1 1 1],1.5);
    ...
    for i = 1:numChoiceTrials
        ...
        [choice,ReactTime,P4Timings.gamble(i)] = GambleScreen(window,cedrus,flip,sure,4);
    ...
    [~,~,~,P4Timings.end(i)] = TextScreen(window,'End of Phase 4',[1 1 1],'key');
...
elseif MRI == 1
    ...
    [~,~,~,S1Timings.instructions] = TextScreen(window, ...
        'Phase 4 Session 1: Please wait for instructions',[1 1 1],'key');
    [~,~,~,S1Timings.getready] = TextScreen(window,'Get Ready',[1 1 1],1.5);
    ...
    for i = 1:numChoiceTrials
        flip = gambleShuffled(i,2);
        sure = gambleShuffled(i,1);
        [choice,ReactTime,S1Timings.gamble(i)] = GambleScreen(window,cedrus,flip,sure,4);
        RestrictKeysForKbCheck([]);
    ...
    [~,~,~,S1Timings.end(i)] = TextScreen(window, ...
        'End of Phase 4 Session 1',[1 1 1],'key');
...
if MRI == 1
    ...
    [~,~,~,S2Timings.instructions] = TextScreen(window, ...
        'Phase 4 Session 2: Please wait for instructions',[1 1 1],'key');
    [~,~,~,S2Timings.getready] = TextScreen(window,'Get Ready',[1 1 1],1.5);
    ...
    for i = 1:numChoiceTrials
        ...
        [choice,ReactTime,S2Timings.gamble(i)] = GambleScreen(window,cedrus,flip,sure,4);
        RestrictKeysForKbCheck([]);
    ...
    [~,~,~,S2Timings.end(i)] = TextScreen(window, ...
        'End of Phase 4 Session 2',[1 1 1],'key');
...
if MRI == 0
    ...
    [~,~,~,P5Timings.instructions] = TextScreen(window, ...
        'Phase 5: Please wait for instructions',[1 1 1],'key');
    ...
    for i = 1:numFatigueTrials
        ...
        [~,~,~,P5Timings.squeezeintro(i)] = TextScreen(window,'Squeeze Phase',[1 1 1],2);
        [~,~,~,P5Timings.squeezegetready(i)] = TextScreen(window,'Get Ready',[1 1 1],1.5);
        % min reps
        for j = 1:minFatigueReps
            [outcome,volt,timing,P5Timings.therm(i,j)] = ThermScreen(window,sensor,baseline,MVC,MVCFatiguePercent/100,0.05,'horizontal',time);
        ...
        [~,~,~,P5Timings.gambleintro(i)] = TextScreen(window,'Gamble Phase',[1 1 1],2);
        [~,~,~,P5Timings.gamblegetready(i)] = TextScreen(window,'Get Ready',[1 1 1],1.5);
        ...
        for k = ChoiceLeftIndex:ChoiceRightIndex
            ...
            [choice,ReactTime,P5Timings.gamble(i,count)] = GambleScreen(window,cedrus,flip,sure,4);
            RestrictKeysForKbCheck([]);
            P5Timings.gamblefixcross(i,count) = FixationCross(window,1+3*rand);
            choiceFatiguedChoiceTrial(k) = choice; %NOTE: 1 = flip, 0 = sure
            reacttimeFatiguedChoiceTrial(k) = ReactTime;
        end
    end
    
    [~,~,~,P5Timings.end] = TextScreen(window,'End of Phase 5',[1 1 1],'key');
...
elseif MRI == 1
    ...
    [~,~,~,S3Timings.instructions] = TextScreen(window, ...
        'Phase 5 Session 3: Please wait for instructions',[1 1 1],'key');
    ...
    for i = 1:numFatigueTrials
        ...
        trial = ['trial' num2str(i)];
        [~,~,~,S3Timings.squeezeintro(i)] = TextScreen(window,'Squeeze Phase',[1 1 1],2);
        [~,~,~,S3Timings.squeezegetready(i)] = TextScreen(window,'Get Ready',[1 1 1],1.5);
        % min reps
        for j = 1:minFatigueReps
            [outcome,volt,timing,S3Timings.therm(i,j)] = ThermScreen(window,sensor,baseline,MVC,MVCFatiguePercent/100,0.05,'horizontal',time);
            voltFatiguedChoiceTrial(j,:,i) = volt;
            timingFatiguedChoiceTrial(j,:,i) = timing;
            outcomeFatiguedChoiceTrial(i,j) = outcome;
            if outcome == 1
        ...
        % Post-Fatigue Choice Trials
        [~,~,~,S3Timings.gambleintro(i)] = TextScreen(window,'Gamble Phase',[1 1 1],2);
        [~,~,~,S3Timings.gamblegetready(i)] = TextScreen(window,'Get Ready',[1 1 1],1.5);
        ChoiceLeftIndex = (i-1)*numChoicesPerTrial+1;
        ChoiceRightIndex = i*numChoicesPerTrial;
        count = 0;
        for k = ChoiceLeftIndex:ChoiceRightIndex
            ...
            flip = gambleShuffled_1(k,2);
            sure = gambleShuffled_1(k,1);
            [choice,ReactTime,S3Timings.gamble(i,count)] = GambleScreen(window,cedrus,flip,sure,4);
            RestrictKeysForKbCheck([]);
            S3Timings.gamblefixcross(i,count) = FixationCross(window,1+3*rand);
            choiceFatiguedChoiceTrial(k) = choice; %NOTE: 1 = flip, 0 = sure
            reacttimeFatiguedChoiceTrial(k) = ReactTime;
        end
    end
    
    [~,~,~,S3Timings.end] = TextScreen(window,'End of Phase 5 Session 3', ...
        [1 1 1],'key');

    FatiguedChoiceTrialSession3 = [gambleShuffled_1(1:numFatiguedChoiceTrials,[1 2]) ...
...
if MRI == 1
    ...
    [~,~,~,S4Timings.instructions] = TextScreen(window, ...
        'Phase 5 Session 4: Please wait for instructions',[1 1 1],'key');

    for i = 1:numFatigueTrials
        minFatigueReps = 5;
        trial = ['trial' num2str(i)];
        [~,~,~,S4Timings.squeezeintro(i)] = TextScreen(window,'Squeeze Phase',[1 1 1],2);
        [~,~,~,S4Timings.squeezegetready(i)] = TextScreen(window,'Get Ready',[1 1 1],1.5);
        % min reps
        for j = 1:minFatigueReps
            [outcome,volt,timing,S4Timings.therm(i,j)] = ThermScreen(window,sensor,baseline,MVC,MVCFatiguePercent/100,0.05,'horizontal',time);
            voltFatiguedChoiceTrial(j,:,i) = volt;
            timingFatiguedChoiceTrial(j,:,i) = timing;
            outcomeFatiguedChoiceTrial(i,j) = outcome;
            if outcome == 1
        ...
        % Post-Fatigue Choice Trials
        [~,~,~,S4Timings.gambleintro(i)] = TextScreen(window,'Gamble Phase',[1 1 1],2);
        [~,~,~,S4Timings.gamblegetready(i)] = TextScreen(window,'Get Ready',[1 1 1],1.5);
        ...
        for k = ChoiceLeftIndex:ChoiceRightIndex
            ...
            flip = gambleShuffled_1(k,2);
            sure = gambleShuffled_1(k,1);
            [choice,ReactTime,S4Timings.gamble(i,count)] = GambleScreen(window,cedrus,flip,sure,4);
            RestrictKeysForKbCheck([]);
            S4Timings.gamblefixcross(i,count) = FixationCross(window,1+3*rand);
            choiceFatiguedChoiceTrial(k) = choice; %NOTE: 1 = flip, 0 = sure
            reacttimeFatiguedChoiceTrial(k) = ReactTime;
        end
    end
    
    [~,~,~,S4Timings.end] = TextScreen(window,'End of Phase 5 Session 4', ...
        [1 1 1],'key');

    FatiguedChoiceTrialSession4 = [gambleShuffled_1(6:6+numFatiguedChoiceTrials,[1 2]) ...
        choiceFatiguedChoiceTrial reacttimeFatiguedChoiceTrial];
    % ^rows-fatigued gamble trial#, column1-sure, column2-flip, column3-choice,
    % column4-reaction time
    
    % save data
    FatiguedChoiceFileName = fullfile(SubjectDir,'FatiguedChoicePhaseSession4');
...
if MRI == 1
    ...
    [~,~,~,S5Timings.instructions] = TextScreen(window, ...
        'Phase 5 Session 5: Please wait for instructions',[1 1 1],'key');

    for i = 1:numFatigueTrials
        minFatigueReps = 5;
        trial = ['trial' num2str(i)];
        [~,~,~,S5Timings.squeezeintro(i)] = TextScreen(window,'Squeeze Phase',[1 1 1],2);
        [~,~,~,S5Timings.squeezegetready(i)] = TextScreen(window,'Get Ready',[1 1 1],1.5);
        % min reps
        for j = 1:minFatigueReps
            [outcome,volt,timing,S5Timings.therm(i,j)] = ThermScreen(window,sensor,baseline,MVC,MVCFatiguePercent/100,0.05,'horizontal',time);
            voltFatiguedChoiceTrial(j,:,i) = volt;
            timingFatiguedChoiceTrial(j,:,i) = timing;
            outcomeFatiguedChoiceTrial(i,j) = outcome;
            if outcome == 1
        ...
        % Post-Fatigue Choice Trials
        [~,~,~,S5Timings.gambleintro(i)] = TextScreen(window,'Gamble Phase',[1 1 1],2);
        [~,~,~,S5Timings.gamblegetready(i)] = TextScreen(window,'Get Ready',[1 1 1],1.5);
        ...
        for k = ChoiceLeftIndex:ChoiceRightIndex
            ...
            flip = gambleShuffled_1(k,2);
            sure = gambleShuffled_1(k,1);
            [choice,ReactTime,S5Timings.gamble(i,count)] = GambleScreen(window,cedrus,flip,sure,4);
            RestrictKeysForKbCheck([]);
            S5Timings.gamblefixcross(i,count) = FixationCross(window,1+3*rand);
            choiceFatiguedChoiceTrial(k) = choice; %NOTE: 1 = flip, 0 = sure
            reacttimeFatiguedChoiceTrial(k) = ReactTime;
        end
    end
    
    [~,~,~,S5Timings.end] = TextScreen(window,'End of Phase 5 Session 5', ...
        [1 1 1],'key');

    FatiguedChoiceTrialSession5 = [gambleShuffled_1(11:11+numFatiguedChoiceTrials,[1 2]) ...
        choiceFatiguedChoiceTrial reacttimeFatiguedChoiceTrial];
    % ^rows-fatigued gamble trial#, column1-sure, column2-flip, column3-choice,
    % column4-reaction time
...
P6Timings.trigger = GetSecs;
[~,~,~,P6Timings.instructions] = TextScreen(window,'Phase 6: Please wait for instructions',[1 1 1],'key');
[~,~,~,P6Timings.getready] = TextScreen(window,'Get Ready',[1 1 1],1.5);
...
for i = 1:length(TrialSelectionTrial)
    ...
    while lever == 0 && FailCount < 5
        ...
        if outcome == 1
            [~,~,~,P6Timings.outcome(i,FailCount+1)] = TextScreen(window,'Success',[0 1 0],2);
            lever = 1;
            if i == length(TrialSelectionTrial)
                break;
            else
                [~,~,~,P6Timings.nextgamble(i)] = TextScreen(window,'Next Gamble',[1 1 1],2);
            end
        elseif outcome == 0
            FailCount = FailCount+1;
            [~,~,~,P6Timings.outcome(i,FailCount)] = TextScreen(window,'Failure',[1 0 0],2);
            if FailCount == 5
                if i == length(TrialSelectionTrial)
                    break;
                else
...
[~,~,~,P6Timings.end] = TextScreen(window,'End of Phase 6',[1 1 1],'key');

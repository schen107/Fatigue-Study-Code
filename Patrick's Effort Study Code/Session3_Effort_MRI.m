%EFFORT EXPERIMENT: Recall Task (PART 2)
load subj_directory;
cd(subj_directory);
%%
...
%--------------------------------------------------------------------------
msg_a='Session 3';msg_b='Please Wait for Instructions';
...
%---RECALL TASK PART 2-----------------------------------------------------
%The recall task is effectively performed twice (once in the previous
%session, and now again in this session).
...
for n=1:length(test_set)
    ...
    for f=1:numFrames_GetReady
        ...
        if f==1
            S3timings.getready(n)=GetSecs-S3timings.trigger;
        end
    end
    clear f
    ...
    for f=1:numFrames_Squeeze
        ...
        vbl=Screen('Flip',window,vbl+(waitframes-0.5)*ifi);
        if f==1
            S3timings.squeeze(n)=GetSecs-S3timings.trigger;
        end
    end
    clear f
...
%--------------------------------------------------------------------------
    ...
    while lev==0&&(t_elapse<=4)
        ...
        if dumdum==1
           S3timings.report(n)=GetSecs-S3timings.trigger;
           dumdum=0; 
        end
...
%--------------------------------------------------------------------------
    ...
    RT_test=(GetSecs-S3timings.trigger)-S3timings.report(n);
    %Get reaction times.
...
%     numSecs_Fixation=fix_interval_min+(fix_interval_max-fix_interval_min)*rand;
    ...
    for f=1:numFrames_Fixation
        ...
        vbl=Screen('Flip',window,vbl+(waitframes-0.5)*ifi);
        if f==1
            S3timings.fix(n)=GetSecs-S3timings.trigger;
        end
    end      
end
clear n
...
clc;
disp('>> SESSION 3 COMPLETE');
disp('>> Ready to run: Session4_Effort_MRI.m');
